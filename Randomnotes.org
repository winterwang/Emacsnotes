...
In CFA models where there are no cross-loading indicators, the complete standardized factor loading can be interpreted as the correlation between the indicator and the factor. Accordingly, squaring the 
completed standardized factor loading provides the proportion of **variance of the indicator** that is explained by the factor -- that is a communality. 
